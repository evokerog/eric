...
// Emoji constants
let enable = `<a:im_tick:1243842036726239303><a:im_Cross:1243842024957018213>`;
let disable = `<a:im_tick:1243842036726239303><a:im_Cross:1243842024957018213>`;
let protect = `<:im_antinuke:1243834677031075901>`;
let hii = `<a:im_dot:1243847404227133510>`;
        ...
        if (message.guild.memberCount < 1) {
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor(0xf7ff00)
                        .setDescription(`${client.emoji.cross} | Your Server Doesn't Meet My 1 Member Criteria`)
                ]
            });
        }
        ...
        if (!option) {
            ...
            if (isActivatedAlready) {
                ...
                if (message.guild.roles.cache.size > 249)
                    return message.reply(`I Won't Able To Create \`Immortals Dominance\` Cause There Are Already 249 Roles In This Server`);
                ...
                let createdRole = await message.guild.roles.create({
                    name: 'Immortals Dominance',
                    position: role ? role : 0,
                    reason: 'Immortals Role For Ubypassable Setup',
